...
def compute_merger_peak(t_ds, hp, hx, a_ds, e_ds, m1_ds, m2_ds, D_obs):
    ...
    h_peak = h_amp * 2.0
    dt = t_ds[-1] - t_ds[-2] if len(t_ds) > 1 else 0.0001
    t_peak = t_ds[-1] + dt
...
class PolarizationPlotter(Plotter):
    ...
    def __init__(self, config, skip_compute=False):
    ...
    def plot(self, run_name):
        ...
        print(f"  ISCO radius: {r_isco / 1000.0:.1f} km")
        print(f"last a: {a_ds[-1]}")
        ...
        if not np.all(isco_mask):
            ...
            print(
                f"  All data is above ISCO (a_min={a_ds[-1] / 1000.0:.1f} km)"
            )
        if not self.skip_compute:
            ...
            print(
                f"  Semi-major axis: {a_ds[0] / 1000.0:.1f} to {a_ds[-1] / 1000.0:.1f} km"
            )
            ...
            hp, hx = get_waveforms(phi_ds, a_ds, e_ds, M_total, mu, self.config.observer_distance)
            ...
            dhp_dt = np.gradient(hp, t_ds)
            dhx_dt = np.gradient(hx, t_ds)
            F_GW = SPEED_OF_LIGHT**3 / (16 * np.pi
                                        * G) * (dhp_dt**2 + dhx_dt**2)
            ...
            hp = f[f"{run_name}/polarizations/h_plus"]
            hx = f[f"{run_name}/polarizations/h_cross"]
            h_amp = f[f"{run_name}/polarizations/h_amplitude"]
            phi_ds = f[f"{run_name}/polarizations/phi"]
            r_ds = f[f"{run_name}/polarizations/r"]
            f_GW = f[f"{run_name}/polarizations/f_GW"]
            F_GW = f[f"{run_name}/polarizations/F_GW"]
            t_ds = f[f"{run_name}/polarizations/times"]
        merger_start_idx, merger_peak_idx = find_merger_region(t_ds, h_amp, threshold_fraction=0.01)
        n_points = len(t_ds)
        ...
        print(
            f"  Interesting region: indices {merger_start_idx} to {merger_peak_idx}"
        )
        ...
        if len(t_focus) > 10:
            print(
                f"Creating focused plots for t=-1 to 0s ({len(t_focus)} points)..."
            )
            ...
            if np.sum(final_mask) > 10:
                ...
                plt.plot(
                    t_final,
                    hp_final * self.config.observer_distance * 100,
                    "b-",
                    linewidth=1,
                )
                plt.xlabel("$t$ [s]", fontsize=18)
                plt.ylabel("$Rh_+$ [cm]", fontsize=18)
                ...
                plt.plot(
                    t_final,
                    hx_final * self.config.observer_distance * 100,
                    "r-",
                    linewidth=1,
                )
                plt.xlabel("$t$ [s]", fontsize=18)
                plt.ylabel("$Rh_\\times [cm]$", fontsize=18)
                plt.gca().yaxis.set_major_formatter(formatter)
                plt.gca().tick_params(axis="both", which="major", labelsize=16)
                plt.xlim(-0.01, 0)
                plt.grid(True, alpha=0.3)
                ...
                plt.plot(
                    t_final,
                    h_amp_final * self.config.observer_distance * 100,
                    "b-",
                    linewidth=1,
                )
                plt.xlabel("$t$ [s]", fontsize=18)
                plt.ylabel("$R|h|$ [cm]", fontsize=18)
                plt.gca().yaxis.set_major_formatter(formatter)
                plt.gca().tick_params(axis="both", which="major", labelsize=16)
                plt.xlim(-1, 0)
                plt.grid(True, alpha=0.3)
                ...
                plt.xlabel("$t$ [s]", fontsize=18)
                plt.ylabel("$f_{GW}$ [Hz]", fontsize=18)
                plt.gca().tick_params(axis="both", which="major", labelsize=16)
                plt.gca().yaxis.set_major_formatter(formatter)
                plt.xlim(-1, 0)
                plt.grid(True, alpha=0.3)
