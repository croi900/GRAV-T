...
def analyze_run(f, run_name):
    ...
    print(
        f"  Duration: {t[-1] - t[0]:.4e} s = {(t[-1] - t[0]) / 3600:.2f} hours"
    )
    ...
    print(
        f"  NaN in a: {np.sum(np.isnan(a))}, NaN in e: {np.sum(np.isnan(e))}"
    )
    ...
    print(f"\n--- Orbital Dynamics ---")
    print(
        f"  GW frequency (Kepler): {f_GW_approx[0]:.2f} to {f_GW_approx[-1]:.2f} Hz"
    )
    print(
