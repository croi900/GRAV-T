...
class MultiPlotter:
    ...
    def saveplot(
        ...
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.legend(loc="best", fontsize=8 * 1.25)
        plt.tight_layout()
    ...
    def generate_plots(self):
        if not self.datasets:
            ...
            return
        print(
            f"Generating comparison plots for {len(self.datasets)} datasets..."
        )
        n = len(self.datasets)
        cmap = plt.cm.viridis
        ...
        labels = [
            f"""${("\\omega" if self.config.decay_type == "exponential" else "k")} = {f"{d['actual_decay_rate']:.2e}".split("e")[0]} \\times 10^{{{int(f"{d['actual_decay_rate']:.2e}".split("e")[1])}}}$ $s^{{-1}}$"""
            for d in all_data
        ]
        AU = 149600000000.0
        ...
        self.saveplot("t_a_comparison", xlabel="$t$ [s]", ylabel="$a$ [AU]")
        plt.figure(figsize=(6, 6))
        for i, data in enumerate(all_data):
            plt.plot(
                data["t_valid_clip"][2:],
        ...
        self.saveplot("t_e_comparison", xlabel="$t$ [s]", ylabel="$e$")
