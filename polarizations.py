...
def compute_phi_r_ode(t, a, e, M):
    ...
    pbar = tqdm(
        total=t[-1] - t[0] + padding,
        desc="  Phase integration",
        unit="s",
        leave=False
    )
    last_t = np.array([t[0]])
    ...
    pbar.close()
    phi_arr = sol.y[0]
    e_safe = np.clip(e, 1e-14, 0.99999999999)
...
def _compute_waveform_point(phi, a, e, G_val, M, mu):
    ...
    phi_dot = L / (r * r)
    phi_ddot = (
        -2 * e_safe * np.sin(phi) * G_val * M * u**3 /
        (a**3 * (1 - e_safe**2)**3)
    )
    cos_phi = np.cos(phi)
    sin_phi = np.sin(phi)
    ...
    d2r_dphi2 = (
        p * e_safe * (cos_phi * u + 2 * e_safe * sin_phi * sin_phi) /
        (u * u * u)
    )
    A = r * r
    dA_dphi = 2 * r * dr_dphi
    ...
    d2Qxx_dphi2 = mu * (
        d2A_dphi2 * Bxx + 2 * dA_dphi * dBxx_dphi + A * d2Bxx_dphi2
    )
    ...
    d2Qyy_dphi2 = mu * (
        d2A_dphi2 * Byy + 2 * dA_dphi * dByy_dphi + A * d2Byy_dphi2
    )
    ...
    d2Qxy_dphi2 = mu * (
        d2A_dphi2 * Bxy + 2 * dA_dphi * dBxy_dphi + A * d2Bxy_dphi2
    )
...
def get_waveforms(phi, a, e, M, mu, D_obs, clight=299800000.0):
...
def get_d2phi_dt2(phi, a, e, G_val, M):
    e_safe = np.clip(e, 1e-10, 0.9999)
    return (
        -2 * e_safe * np.sin(phi) * G_val * M * (1 + e_safe * np.cos(phi))**3 /
        (a**3 * (1 - e_safe**2)**3)
    )
