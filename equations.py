...
class AnalyticMassFunction:
    ...
    def evaluate_all(self, t: float) -> Tuple[float, float, float, float]:
        return (
            _ensure_float(self.value,
                          t),
            _ensure_float(self.first,
                          t),
            _ensure_float(self.second,
                          t),
            _ensure_float(self.third,
                          t),
        )
...
def _combine_scalings(
    ...
    f_mu, df_mu, d2f_mu, d3f_mu = _compute_fmu(M_c, M_c1, M_c2, f1, df1, d2f1, d3f1, f2, df2, d2f2,
                                               d3f2)
...
def _F_factors(f_M, f_mu, df_M, df_mu, d2f_M, d2f_mu, d3f_mu):
    ...
    F2 = (
        sqrt_f_M * 3.0 * d2f_mu + 3.0 * df_mu * df_M /
        (2.0 * sqrt_f_M) + f_mu / (4.0 * f_M**1.5) *
        (2.0 * d2f_M * f_M - df_M**2)
    )
...
def _classical_dEdt(mu_c, M_c, a, e):
    ...
    prefactor = 8.0 * G**4 * M1_sq_M2_sq_M / (
        15.0 * c**5 * a**5 * sqrt_1_e2**10
    )
...
def _classical_dLdt(mu_c, M_c, a, e):
    prefactor = (
        32.0 * G**3.5 * np.sqrt(M_c) * mu_c**2 * M_c /
...
def _variable_mass_dEdt(
    ...
    sqrt_1_e2 = np.sqrt(1.0 - e**2)
    correction_1 = (
        16.0 * G**2 * M_c**2 * (4.0 - sqrt_1_e2) / (a**2 * sqrt_1_e2) *
        (3.0 * F3**2 - 2.0 * F2 * F4)
    )
    correction_2 = 4.0 * G * M_c * a * (3.0 - e**
                                        2) * (3.0 * F2**2 - 2.0 * F1 * F3)
...
def _variable_mass_dLdt(
    ...
    Lz_correction_1 = (
        sqrt_factor * 8.0 * G * M_c *
        (Fp1 * F4 + 3.0 * Fp3 * F2 - 6.0 * Fp2 * F3)
    )
    Lz_correction_2 = (
        sqrt_factor * a**3 * (2.0 + 3.0 * e**2) *
        (2.0 * Fp2 * F1 - 3.0 * Fp1 * F2)
    )
...
def _dadt(
    ...
    correction = (
        mu_c / (5.0 * c**5 * f1 * f2) * (
            ...
            (3.0 * F3**2 - 2.0 * F2 * F4) + 4.0 * G * a**3 * (3.0 - e**2) *
            (3.0 * F2**2 - 2.0 * F1 * F3) + a**6 / (6.0 * M_c) *
            (8.0 + 40.0 * e**2 + 15.0 * e**4) * F1**2
        )
    )
...
def _dedt(
    ...
    Fp1, Fp2, Fp3 = _Fp_factors(f_M, f_mu, df_M, df_mu, d3f_mu)
    dedt_classical = (
        -304 * G**3 * M_c1 * M_c2 * M_c * e /
        (15.0 * c**5 * a**4 * sqrt_1_e2**5) * (1 + 121 / 304 * e_sq)
    )
    correction_1 = (
        mu_c * a * (1.0 - e_sq) / (10.0 * c**5 * f1 * f2) * inv_e * (
            16.0 * G**2 * M_c * (4.0 - sqrt_1_e2) / (a**2 * sqrt_1_e2) *
            (3.0 * F3**2 - 2.0 * F2 * F4) + 4.0 * G * a * (3.0 - e_sq) *
            (3.0 * F2**2 - 2.0 * F1 * F3) + a**4 / (6.0 * M_c) *
            (8.0 + 40.0 * e_sq + 15.0 * e_sq**2) * F1**2
        )
    )
    correction_2 = (
        G * mu_c * (1.0 - e_sq) / (5.0 * c**5 * a) * inv_e * np.sqrt(f_M) /
        (f1 * f2) * (
            8.0 * G * M_c *
            (Fp1 * F4 + 3.0 * Fp3 * F2 - 6.0 * Fp2 * F3) + a**3 *
            (2.0 + 3.0 * e_sq) * (2.0 * Fp2 * F1 - 3.0 * Fp1 * F2)
        )
    )
    return (
        F4**2 / (f1 * f2) * dedt_classical - correction_1 - correction_2 -
        (1.0 - e_sq) / 2.0 * inv_e *
        (df_M / f_M - 3.0 * df1 / f1 - 3.0 * df2 / f2)
    )
...
def _dPdt(
    ...
    dPdt_val = (
        2 * np.pi / np.sqrt(G * M_c) * np.sqrt(a / f_M) *
...
class BinarySystemModelFast:
    ...
    def __init__(self, config) -> None:
        ...
        if isinstance(config, cfg.Config):
            ...
            self.mass_fun_1 = decay_map(
                config.decay_type,
                config.state.decay_rate
            )
            self.mass_fun_2 = decay_map(
                config.decay_type,
                config.state.decay_rate
            )
    ...
    def _snapshot(self, t: float) -> ScalingSnapshot:
        ...
        f_M, df_M, d2f_M, f_mu, df_mu, d2f_mu, d3f_mu = _combine_scalings(
            self.M_c1, self.M_c2, self.M_c, f1, df1, d2f1, d3f1, f2, df2, d2f2, d3f2)
    ...
    def coalescence_time(
        ...
        if solution.t_events[0].size > 0:
            ...
            print(
                f"Mass depletion limit reached at t={solution.t_events[1][0]:.4e} s"
            )
            return (solution.t_events[1][0], solution)
        else:
            print(
                "Semi-major axis did not reach minimum for the allowed timeframe."
            )
