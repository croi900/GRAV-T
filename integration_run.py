...
class IntegrationRun:
    ...
    def __init__(
        ...
        self.t0 = t0
        self.t1 = t1
        self.writer = h5py.File(
            f"{self.config.name}/{self.config.name}.h5",
            "a"
        )
    ...
    def run(self):
        with self.writer as writer:
            vprint(
                f"creating datasets in h5 {self.config.name}: {self.name}/.... "
            )
            ...
            def mass_scale_ok(t):
                return self.decay_function.value(t) > 1e-06
            ...
            with tqdm(
                total=t_eval.shape[0],
                desc=f"{self.name} ODE: "
            ) as pbar:
                while self.solver.t < self.t1:
                    if not mass_scale_ok(self.solver.t):
                        vprint(
                            "mass scaling below threshold, stopping integration"
                        )
                        break
                    if t_eval_ptr >= len(t_eval):
                        vprint(
                            "t_eval outside of t_eval, stopping integration"
                        )
                        break
                    ...
                    while t_target <= self.solver.t:
                        ...
                        m1_buffer[buffer_ptr] = (
                            self.state.M1
                            * self.decay_function.value(t_target)
                        )
