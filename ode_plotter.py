...
class ODEPlotter(Plotter):
    ...
    def plot(self, run_name):
        ...
        valid_mask = e_ds > 1e-06
        t_valid = t_ds[valid_mask]
        a_valid = a_ds[valid_mask]
        e_valid = e_ds[valid_mask]
        m1_valid = m1_ds[valid_mask]
        m2_valid = m2_ds[valid_mask]
        plt.plot(t_ds, a_ds / 1000.0)
        self.saveplot(f"{run_name}/t_a", xlabel="$t$ [s]", ylabel="$a$ [km]")
        plt.plot(t_valid, e_valid)
        self.saveplot(f"{run_name}/t_e", xlabel="$t$ [s]", ylabel="$e$")
        plt.plot(t_ds, m1_ds)
        self.saveplot(
            f"{run_name}/t_m1",
            xlabel="$t$ [s]",
